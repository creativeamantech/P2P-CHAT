                            ...
                            when (msg) {
                                ...
                                is TransportMessage.Reaction -> {
                                    ...
                                    if (currentMessage != null) {
                                        try {
                                            // Simple JSON manipulation for MVP to avoid adding Gson/Moshi dep just here
                                            // Parse JSON: {"senderId":"emoji","senderId2":"emoji2"}
                                            ...
                                            if (msg.remove) {
                                                ...
                                                map[peerId] = msg.emoji
                                            }

                                            // Serialize back to JSON
                                            val newJson = "{" + map.entries.joinToString(",") { "\"${it.key}\":\"${it.value}\"" } + "}"
                                            messageRepository.updateReactions(msg.messageId, newJson)

                                            Log.d("MessageProcessor", "Updated reactions for ${msg.messageId}: $newJson")
                                        } catch (e: Exception) {
                                            Log.e("MessageProcessor", "Failed to parse/update reactions JSON", e)
                                        }
                                    }
                                }
